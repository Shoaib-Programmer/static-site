    ...
    for (const block of blocks) {
        ...
        switch (type) {
            case BlockType.HEADING: {
                ...
                const content = match ? (match[2] as string) : block.replace(/^#+\s*/, "");
                children.push(new ParentNode(`h${level}`, textToChildren(content), {}));
                break;
            }
            case BlockType.CODE: {
                const codeMatch = block.match(/^```(?:\w+)?\n?([\s\S]*?)\n?```$/);
                const codeContent = codeMatch ? (codeMatch[1] as string) : block;
                const codeLeaf = textNodeToHTMLNode(new TextNode(codeContent, TextType.CODE));
            ...
            case BlockType.QUOTE: {
                const quoteText = block
                    ...
                    .join("\n");
                children.push(new ParentNode("blockquote", textToChildren(quoteText), {}));
                break;
            }
            case BlockType.UNORDERED_LIST: {
                const items = block.split("\n").map((line) => line.replace(/^\-\s+/, ""));
                const liNodes = items.map((item) => new ParentNode("li", textToChildren(item), {}));
