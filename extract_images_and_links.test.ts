...
describe("helpers: extractMarkdownImages", () => {
	...
	test("finds multiple images in a string", () => {
		const input = "Start ![one](http://a.com/1.png) middle ![two](http://b.com/2.jpg) end";
	...
	test("supports empty alt text", () => {
		...
		expect(extractMarkdownImages(input)).toEqual([["", "http://a.com/x.png"]]);
	});

	test("alt text with spaces and punctuation", () => {
		const input = "Look ![an example image! v2](https://cdn.ex/x-y_z.png) now";
		expect(extractMarkdownImages(input)).toEqual([
	...
	test("URL containing parentheses is captured entirely", () => {
		...
		expect(extractMarkdownImages(input)).toEqual([["pic", "https://ex.com/image_(final).png"]]);
	});

	test("does not match non-image markdown links", () => {
		const input = "a [link](https://ex.com) and ![img](https://ex.com/i.png)";
		expect(extractMarkdownImages(input)).toEqual([["img", "https://ex.com/i.png"]]);
	});
...
describe("helpers: extractMarkdownLinks", () => {
	...
	test("finds multiple links in order", () => {
		const input = "[one](http://a) and [two](http://b) and [three](http://c)";
	...
	test("link text can include spaces and punctuation", () => {
		...
		expect(extractMarkdownLinks(input)).toEqual([["My Site, Inc.", "https://ex.com/home"]]);
	});
	...
	test("URL containing parentheses is captured entirely", () => {
		...
		expect(extractMarkdownLinks(input)).toEqual([["k", "https://ex.com/file_(1).zip"]]);
	});
