...
describe("ParentNode", () => {
	...
	test("toHTML with grandchildren (nested ParentNode)", () => {
		...
		const parentNode = new ParentNode("div", [childNode], {});
		expect(parentNode.toHTML()).toBe("<div ><span ><b >grandchild</b></span></div>");
	});
	...
	test("toHTML with multiple children", () => {
		...
		expect(parentNode.toHTML()).toBe("<div ><span >one</span><span >two</span></div>");
	});
	...
	test("throws when children is undefined", () => {
		...
		expect(() => parentNode.toHTML()).toThrow("Children must be present for a Parent");
	});

	test("renders attributes when props provided", () => {
		const childNode = new LeafNode("span", "child", {});
		const parentNode = new ParentNode("div", [childNode], { class: "container", id: "root" });
		expect(parentNode.toHTML()).toBe("<div class=container id=root ><span >child</span></div>");
	});
});
